...
public class MediaPlayerCtrl : MonoBehaviour {
	...
	public Texture2D m_VideoLogoTexture;
	...
	private WWW		download = null;
	private WWW 	audioDownload = null;
#if UNITY_IPHONE
	...
	private bool m_downloadError = false;
	...
	void Update()
	{
		if (download != null) {
			...
			if( download.progress != 1 ) {
				//Call_UnityLog ("Downloading " + download.progress);
				Call_DownloadProgress(download.progress);
			}
		}
		...
		if(m_bFirst == false)
		{
			...
			if(m_bSupportRockchip)
			{
				...
				{
					//Call_Load(strName,0);
					Call_SetAudioPath(m_strAudioFileName);
		...
		if(m_CurrentState != Call_GetStatus() )
		{
			...
			{
				GetComponent<Renderer> ().material.mainTexture = m_VideoLogoTexture;
				if(OnReady != null)
					OnReady();
    ...
    private AndroidJavaObject GetJavaObject()
    {
        if (javaObj == null)
        {
			javaObj = new AndroidJavaObject("com.sogwangtec.beijingsunyong.VideoTexture");
        }
	...
	private void Call_HeadLog() {
		Vector3 v = Cardboard.SDK.HeadRotation.eulerAngles;
		GetJavaObject().Call("HeadLog", v.x, v.y, v.z);
	}

	private void Call_UnityLog(string log) {
		GetJavaObject().Call("UnityLog", log);
	}

	private void Call_SetAudioPath(string url) {
		GetJavaObject().Call("SetAudioPath", url);
	}

	private void Call_DownloadError() {
		GetJavaObject().Call("DownloadError");
	}

	private void Call_DownloadProgress(float value) {
		GetJavaObject().Call("downloadProgress", value);
	}
	...
	IEnumerator DownloadStreamingVideoAndLoad(string strURL)
	{
		...
		//Call_UnityLog(Application.persistentDataPath);
		download = new WWW (strURL);
		...
		*/
			System.IO.File.WriteAllBytes(write_path, download.bytes);
			
			//Call_Load("file://"+write_path,0);
			m_strFileName = "file://" + write_path;
		}
		else
		{
			Call_DownloadError();
	...
	IEnumerator DownloadStreamingAudioAndLoad(string strURL)
	{
		strURL = strURL.Trim();
		
		//Call_UnityLog(Application.persistentDataPath);
		audioDownload = new WWW (strURL);
		
		yield return audioDownload;
		...
		*/
			System.IO.File.WriteAllBytes(write_path, audioDownload.bytes);
			
			//Call_Load("file://"+write_path,0);
			m_strAudioFileName = "file://" + write_path;
		}
		else
		{
			Call_DownloadError();
	...
	public void DownloadVideo(string url)
	{
		...
		StopCoroutine ("DownloadStreamingVideoAndLoad");

		GetComponent<Renderer> ().material.mainTexture = m_VideoLogoTexture;

		string video_path = Application.persistentDataPath + "/Data" + url.Substring(url.LastIndexOf("/"));
		if (System.IO.File.Exists (video_path) == false) {
			StartCoroutine (DownloadStreamingVideoAndLoad (url));
		} else {
			m_strFileName = "file://" + video_path;
		}
	...
	{
		if (download != null || audioDownload != null)
			return;
		
		m_strAudioFileName = "";

		StopCoroutine ("DownloadStreamingAudioAndLoad");
		
		GetComponent<Renderer> ().material.mainTexture = m_VideoLogoTexture;
		
		string audio_path = Application.persistentDataPath + "/Data" + url.Substring(url.LastIndexOf("/"));
		if (System.IO.File.Exists (audio_path) == false) {
			StartCoroutine (DownloadStreamingAudioAndLoad (url));
		} else {
			m_strAudioFileName = "file://" + audio_path;
		}

		Call_SetAudioPath("file://" + audio_path);
		//m_strFileName = url;
		//m_strFileName = url;
		//m_bFirst = false;
		//GetComponent<Renderer> ().material.mainTexture = m_VideoLogoTexture;
